...
function CodeBlock() {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 shadow-2xl shadow-slate-950/15">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
        <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Curl</span>
...
function ProductPreview() {
  return (
    <div className="relative rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-2xl shadow-blue-950/10">
      <div className="absolute -right-10 -top-10 size-28 rounded-full bg-blue-200/60 blur-3xl" />
      <div className="absolute -bottom-10 left-10 size-28 rounded-full bg-violet-200/50 blur-3xl" />
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
          <div className="flex items-center gap-2">
            <span className="grid size-7 place-items-center rounded-lg bg-slate-950 text-xs font-bold text-white">S</span>
            <span className="text-sm font-semibold text-slate-950">SLAI console</span>
          </div>
          <span className="rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">Prepaid</span>
        </div>
          ...
          {["Balance", "API key", "Usage", "Ledger"].map((label, index) => (
            <div key={label} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 h-2 rounded-full bg-slate-100">
                <div className="h-2 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500" style={{ width: `${72 - index * 12}%` }} />
              </div>
        ...
        <div className="border-t border-slate-200 bg-white p-4">
          <div className="rounded-xl bg-slate-950 p-4 font-mono text-xs leading-6 text-slate-100">
            <span className="text-blue-300">Authorization:</span> Bearer YOUR_SLAI_API_KEY
          </div>
        </div>
      </div>
    </div>
...
function StepCard({ index, title, text }: { index: number; title: string; text: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <span className="grid size-9 place-items-center rounded-xl bg-blue-50 text-sm font-bold text-blue-700 ring-1 ring-blue-100">{index}</span>
...
function FeatureCard({ title, text, index }: { title: string; text: string; index: number }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-blue-200 hover:shadow-md">
      <span className="grid size-10 place-items-center rounded-xl bg-slate-950 text-sm font-bold text-white">{String(index + 1).padStart(2, "0")}</span>
      <h3 className="mt-5 text-base font-semibold text-slate-950">{title}</h3>
      <p className="mt-2 text-sm leading-6 text-slate-500">{text}</p>
    </div>
  );
    ...
    <AppShell section="public">
          ...
          <div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {trustBullets.map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white/75 px-3 py-2 text-sm font-medium text-slate-700 shadow-sm backdrop-blur">
      ...
      <section id="how-it-works" className="mt-16 scroll-mt-24">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 text-3xl font-semibold tracking-normal text-slate-950">From top-up to tracked usage</h2>
          <p className="mt-3 text-sm leading-6 text-slate-500">SLAI keeps the developer flow small: fund an account, use one key, and let synced usage settle against a ledger-backed balance.</p>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {steps.map((step, index) => <StepCard index={index + 1} key={step.title} title={step.title} text={step.text} />)}
        </div>
      </section>

      <section className="mt-16">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-700">Platform</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-normal text-slate-950">Built for prepaid AI operations</h2>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {features.map(([title, text], index) => <FeatureCard index={index} key={title} title={title} text={text} />)}
        </div>
      </section>

      <section className="mt-16 grid gap-8 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm lg:grid-cols-[0.85fr_1.15fr] lg:p-8">
        <div>
      ...
      <section className="mt-16 rounded-[2rem] bg-slate-950 p-8 text-white shadow-2xl shadow-slate-950/15 lg:p-10">
